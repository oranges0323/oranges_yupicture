...
public class StpInterfaceImpl implements StpInterface {
    ...
    /**
     * 从请求中获取上下文对象  就是获取他们url前缀，判断是图片还是空间操作
     */
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        if (ContentType.JSON.getValue().equals(contentType)) {
            ...
            Map<String, String[]> paramMap = ServletUtil.getParams(request);

            // 创建一个新的参数Map，用于处理可能包含方括号的ID参数
            ...
            for (Map.Entry<String, String[]> entry : paramMap.entrySet()) {
                ...
                if (values != null && values.length > 0) {
                    // 处理可能包含方括号的ID参数
                    if ((key.equals("id") || key.equals("spaceId") || key.equals("pictureId") || key.equals("spaceUserId")) 
                            && values[0].startsWith("[") && values[0].endsWith("]")) {
                        // 去除方括号
                        String cleanedValue = values[0].substring(1, values[0].length() - 1);
                        processedParams.put(key, cleanedValue);
                    } else {
                        // 保留原始值
                        processedParams.put(key, values[0]);
                    }
                }
            }

            authRequest = BeanUtil.toBean(processedParams, SpaceUserAuthContext.class);
        }
        ...
        if (ObjUtil.isNotNull(id)) {
            ...
            //获取前缀的第一个斜杠前的字符串
            StrUtil.subBefore(partURL, "/", false);
            // 获取前缀的第一个斜杠前的字符串
 ...
 * @return 如果对象为null或所有字段都为空，则返回true；否则返回false
 */
